...
void Excersize_2_22(int solution, int answer)
{
	...
	if (answer == SHOW)
	{
		...
		printf("V = V0 + a * t = V0 + g * t = %.0lf m/s + (%.1lf m/s^2) * (%.0lf s) = %.1lf m/s 값이 나옵니다.\n\n", -V0, -g, t, -V); //중력가속도 공식으로 변환 후 각 항에 대응되는 값을 대입 한 후 문제 풀이
		printf("*****************************************************************\n");
		printf("\n\n");
	}
	...
	if (solution == SHOW)
	{
		...
		printf("y : 높이, y0 :처음높이, V0 : 처음속도, a : 가속도,  t : 시간입니다.\n");
		printf("자유낙하 운동이므로 가속도(a)는 중력가속도(g)로 변환됩니다.\n");
		printf("연직 아래 방향이므로 중력가속도(g)와 처음속도(V0)은 음수값을 갖게 됩니다.\n");
		printf("중력가속도는 %.1lf m/s^2 입니다. 시간은 %.0lf s 입니다. 처음높이는 %.0lf m 입니다. V0는 처음 속도이므로 %.0lf m/s 입니다.\n", -g, t, y0, -V0 );
		printf("관련공식에 중력가속도를 대입하면 y = y0 + V0 * t + 1/2 * g * t^2으로 식 변환이 됩니다.\n");
		printf("변환된 식에 각각의 항에 대응하는 값을 대입해주면\n");
		printf("y = y0 + V0 * t + 1/2 * g * t^2 = % .0lf + (%.0lf m/s * %.0lf s) + 1/2 * (%.1lf m/s^2) * (%.0lf s)^2 가 됩니다.\n", y0, -V0, t, -g, t);
	...
	if (answer == SHOW)
	{
		...
		y = V0 * t + (g * t);
		printf("풀이과정에서 정리된 공식을 계산하면\n\n");
		printf("y = y0 + V0 * t + 1/2 * a * t^2 = y0 + V0 * t + 1/2 * g * t^2\n\n"); //중력가속도 공식으로 변환
		printf("  = %.0lf + (%.1lf m/s * %.0lf s) + 1/2 * (%.1lf m/s^2) * (%.0lf s)^2\n\n", y0, -V0, t, -g, t); //각 항에 대응되는 값을 대입 한 후 문제 풀이
		printf("  = (%.1lf m/s * %.0lf s) + 1/2 * (%.1lf m/s^2) * (%.0lf s)^2 = %.1lf m 값이 나옵니다. \n\n", -V0, t, -g, t, -y); //각 항에 대응되는 값을 대입 한 후 문제 풀이
		printf("*****************************************************************\n\n\n");
