...
void Excersize_2_22(int solution, int answer)
{
	...
	if (answer == SHOW)
	{
	    ...
	    printf("V = -%.1lf m + (- %.1lf m/s^2) * %.0lf s 식을 계산하면 \n\n", V0, g, t);
		printf("-%.1lf m + (- %.1lf m/s^2) * %.0lf s = -%.1lf m/s 값이 나옵니다. \n\n", V0, g, t, V);
		printf("*****************************************************************\n");
		printf("\n\n");
	}
	...
	if (solution == SHOW)
	{
		...
		printf("추가적으로 y = y0 + V0 * t + 1/2 * g * t^2 식에서 y0 값이 %.0lf 이므로 식에서 제외해주면\n", y0);
		printf("y = V0 * t + 1/2 * g * t^2 식이 됩니다.\n");
		printf("이후에 식을 정리하여 높이값을 구해주면 됩니다.\n");
		printf("정리된 식은 y = -%.1lf m/s * %.0lf s + 1/2 * (-%.1lf m/s^2) * (%.0lf^2 s) 가 됩니다.\n\n", V0, t, g, t);
		printf("*****************************************************************\n");
		printf("\n\n");
	}
	...
	if (answer == SHOW) 
	{
		...
		y = V0 * t + (g * t);
		printf("풀이과정에서 정리된 공식인\n\n"); 
		printf("y = -%.1lf m/s * %.0lf s + 1/2 * (-%.1lf m/s^2) * (%.0lf^2 s) 식을 계산하면\n\n", V0, t, g, t);
