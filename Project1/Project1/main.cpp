...
void Excersize_2_22(int solution, int answer)
{
	...
	printf("(a) 공을 던진 후 %.0f s 후의 속도를 구하라.\n\n", t);
	...
	if (solution == SHOW)
	{
		...
		printf("V = %.0lf m/s + ( %.1lf m/s^2) * (%.0lf s)\n", -v0, -g, t); 
		printf("V = %.1lf m/s\n\n", -V);
	...
	if (answer == SHOW)
	{
		...
		printf("=========================================   정 답   =========================================\n\n");
		V = v0 + (g * t); 
		printf("공을 던진 후 %.0f s 후의 속도 = %.1lfm/s\n", t, -V);
	...
	if (solution == SHOW)
	{
		...
		printf("y = %.0lf + (%.1lf m/s * %.0lf s) + 1/2 * (%.1lf m/s^2) * (%.0lf s)^2\n", y0, -v0, t, -g, t); //각 항에 대응되는 값을 대입 한 후 문제 풀이 
		printf("y = %.1lf m\n\n", -y);
