...
void Excersize_3_11(int solution, int answer) //한라대학교 미래모빌리티공학과 신현서
{
    ...
    if (solution == SHOW)
    {
        ...
        printf("Δy = y - y0, y : 나중거리, y0 : 초기거리\n");
        printf("v0y : y의 초기속도, a : 가속도, t : 시간\n\n");
...
void Excersize_3_21(int solution, int answer)
{
    ...
    double mass = 7.3 + rand() % (3 + 1);
    double angle = 40 + rand() % (5 + 1);
...
void Excersize_3_25(int solution, int answer) //한라대학교 미래모빌리티공학과 신현서
{
    ...
    double height = 0;
    double g = 9.80;
    double degree = 45 + rand() % (7 + 1) - 1;
    double velocity_0 = 28 + rand() % (5 + 1) - 1;
    double v0y;
    double height_0 = 1.6 + rand() % (1 + 1) - 1;
    double delta_x;
    double time_1;
    double a;
    double b;
    ...
    if (solution == SHOW)
    {
        ...
        v0y = velocity_0 * (sin(DEG2RAD(degree)));
    ...
    if (solution == SHOW)
    {
        ...
        // your code here
        printf("관련 공식은 Δx = v0x * t\n");
        printf("Δx = x - x0, x : 나중거리, x0 : 초기거리, v0x : x의 초기속도\n");
        printf("cosθ : 삼각함수에서 x 성분, v0 : 초기속도, t : 시간\n\n");

        delta_x = velocity_0 * (cos(DEG2RAD(degree))) * time_1;
...
int main(void)
{
    ...
    //Excersize_3_21(Show_Solution, Answer);    //한라대학교 미래모빌리티공학과 송의준
    //Excersize_3_23(Show_Solution, Answer);    //한라대학교 미래모빌리티공학과 한석희
